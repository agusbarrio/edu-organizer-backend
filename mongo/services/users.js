...
const usersServices = {
    ...
    getMyUser: async function ({ user }) {
        const myUser = await userRepositories.getOneById(user._id)
        return myUser
    },
    editMyUser: async function ({ firstName, lastName, user }) {
        await userRepositories.editOneById(user._id, { firstName, lastName })
    },
    changePassword: async function ({ newPassword, user }) {
        ...
        await userRepositories.editOneById(user._id, { password: encryptedPassword })
    }
}
